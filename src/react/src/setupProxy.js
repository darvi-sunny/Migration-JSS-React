...
module.exports = (app) => {
  if (isDisconnected()) {
    ...
    app.use(
      createProxyMiddleware('/sitecore', { target: config.sitecoreApiHost, changeOrigin: true })
    );
    // media items
    app.use(createProxyMiddleware('/-', { target: config.sitecoreApiHost, changeOrigin: true }));
    // visitor identification
    app.use(
      createProxyMiddleware('/layouts', { target: config.sitecoreApiHost, changeOrigin: true })
    );
  }
};
